              ...
              return (
                <Link className="transaction-row" href={`/admin/transactions/${transaction.id}`} key={transaction.id}>
                  <div className="transaction-primary">
                    ...
                    <time>{new Intl.DateTimeFormat("vi-VN", { dateStyle: "short", timeStyle: "short", timeZone: "Asia/Ho_Chi_Minh" }).format(transaction.createdAt)}</time>
                  </div>
                  <div className="transaction-user"><strong>{transaction.user.name}</strong><span>{transaction.items.length} khoản</span></div>
                  <div className="transaction-sessions"><strong>{sessionNames[0] ?? "Không có khoản thu"}</strong>{sessionNames.length > 1 ? <span>+{sessionNames.length - 1} khoản khác</span> : transaction.items[0] ? <span>{new Intl.DateTimeFormat("vi-VN", { dateStyle: "short", timeZone: "Asia/Ho_Chi_Minh" }).format(transaction.items[0].sessionMember.session.playedAt)}</span> : null}</div>

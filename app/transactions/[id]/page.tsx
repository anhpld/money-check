    ...
    <AdminShell active="transactions">
      <div className="page-content transaction-detail-page">
        ...
        <section className="panel transaction-items-panel">
          ...
          <div className="table-wrap"><table><thead><tr><th>Khoản thu</th><th>Ngày</th><th>Khoản chính</th><th>Tùy chọn</th><th>Tổng</th></tr></thead><tbody>{transaction.items.map((item) => <tr key={item.id}><td><strong>{item.sessionMember.session.title}</strong></td><td>{new Intl.DateTimeFormat("vi-VN", { dateStyle: "short", timeZone: "UTC" }).format(item.sessionMember.session.playedAt)}</td><td>{formatVnd(item.footballAmount)}</td><td>{item.options.length ? <div className="transaction-option-list">{item.options.map((option) => <span key={option.id}>{option.name}: <strong>{formatVnd(option.amount)}</strong></span>)}</div> : "—"}</td><td><strong className="amount-emphasis">{formatVnd(item.expectedAmount)}</strong></td></tr>)}</tbody></table></div>
        </section>
      </div>
    </AdminShell>
  );

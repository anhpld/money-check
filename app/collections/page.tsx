    ...
    <AdminShell active="collections">
      <div className="page-content">
        <div className="page-heading collection-page-heading">
          ...
          <Link className="primary-button top-add-button" href="/collections/new">
        ...
        {sessions.length ? (
            ...
            {sessions.map((session) => {
              ...
              return (
                <article className="session-card panel" key={session.id}>
                  ...
                  <div className="session-money"><span>Tổng tiền<strong>{formatVnd(session.totalAmount)}</strong></span><span>Đã phân bổ<strong>{formatVnd(allocated)}</strong></span></div>
                  <div className="session-progress"><span style={{ width: `${allocated ? Math.min((paid / allocated) * 100, 100) : 0}%` }} /></div>
                  <div className="session-card-bottom">
                    <span>{session.members.length} người · Còn {formatVnd(outstanding)}</span>
                    <Link href={`/collections/${session.id}`}>{session.status === "DRAFT" ? "Tiếp tục" : "Xem & sửa"}<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m9 18 6-6-6-6" /></svg></Link>
                  </div>
                </article>
              );
            })}
          ...
          <div className="panel collections-empty">
            ...
            <Link className="primary-button top-add-button" href="/collections/new">Tạo khoản thu đầu tiên</Link>
          </div>
        )}
      </div>
    </AdminShell>
